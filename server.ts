...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    if (!process.env.GEMINI_API_KEY || !ai) {
      return res.json({
        reply: "Hello! I am Vishwa's AI Assistant. Currently, the `GEMINI_API_KEY` is not set on the server. Please add it under 'Settings > Secrets' in AI Studio, or feel free to browse the portfolio directly to see my projects and skills!"
      });
    }
